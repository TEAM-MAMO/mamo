...
export interface MainVisualCardProps
...
export const MainVisualCard = React.forwardRef<
  HTMLDivElement,
  MainVisualCardProps
>(
  (
      ...
      <div ref={ref} className={clsx(s.visualStyle, className)} {...props}>
        <div className={s.headerStyle}>
          ...
          <div className={s.sloganStyle}>
            ...
            <Heading level={2}>
              <span className={s.highlightStyle}>{productName}</span>
              &nbsp;선물하기
            </Heading>
          </div>
          <button
            className={s.moreStyle}
            aria-label="more"
            onClick={onClickMore}
          >
        ...
        <ul className={s.paginationStyle}>
          {productImages.map((_, idx) => (
            <li
              className={s.bulletStyle({ active: idx === currentIdx })}
              key={idx}
            />
          ))}
        </ul>
